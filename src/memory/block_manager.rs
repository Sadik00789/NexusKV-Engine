...
pub struct RadixTrieNode {
    ...
    pub children: HashMap<Vec<u32>, usize>,
}
...
impl RadixPrefixCache {
    pub fn new() -> Self {
        Self {
            nodes: vec![RadixTrieNode::default()],
        }
    }
...
impl BlockSpaceManager {
    ...
    pub fn free_block(&mut self, block_id: usize) -> Result<(), MemoryError> {
        ...
        if block.ref_count == 0 {
            ...
            block.is_prefix_cached = false;
            block.is_speculative = false;
            if !self.free_stack.contains(&block_id) {
    ...
    pub fn free_blocks_count(&self) -> usize {
        self.free_stack.len().min(self.total_blocks)
    }

    pub fn allocated_blocks_count(&self) -> usize {
        self.total_blocks.saturating_sub(self.free_stack.len())
    }
    ...
    pub fn memory_usage_ratio(&self) -> f64 {
        if self.total_blocks == 0 {
            ...
            let allocated = self.allocated_blocks_count() as f64;
            (allocated / self.total_blocks as f64).clamp(0.0, 1.0)
        }
    }
    ...
    pub fn prefix_cache_hit_ratio(&self) -> f64 {
        if self.prefix_cache_lookups == 0 {
            ...
            (self.prefix_cache_hits as f64 / self.prefix_cache_lookups as f64).clamp(0.0, 1.0)
        }
    }
}
